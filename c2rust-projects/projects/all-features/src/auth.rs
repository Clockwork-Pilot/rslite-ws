...
unsafe extern "C" fn sqliteAuthBadReturnCode(mut pParse: *mut Parse) {
    unsafe {
        sqlite3ErrorMsg(
            pParse,
            b"authorizer malfunction\0".as_ptr() as *const ::core::ffi::c_char,
        );
...
pub unsafe extern "C" fn sqlite3AuthReadCol(
    ...
    unsafe {
        ...
        rc = (*db)
            ...
            )((*db).pAuthArg, SQLITE_READ, zTab, zCol, zDb, (*pParse).zAuthContext);
        if rc == SQLITE_DENY {
            ...
            if (*db).nDb > 2 as ::core::ffi::c_int || iDb != 0 as ::core::ffi::c_int {
                z = sqlite3_mprintf(
                    b"%s.%z\0".as_ptr() as *const ::core::ffi::c_char,
                    zDb,
                    z,
                );
            }
            sqlite3ErrorMsg(
                pParse,
                b"access to %z is prohibited\0".as_ptr() as *const ::core::ffi::c_char,
                z,
            );
    ...
    unsafe {
        ...
        if (*pExpr).op as ::core::ffi::c_int == TK_TRIGGER {
            ...
            while iSrc < (*pTabList).nSrc {
                if (*pExpr).iTable
                    == (*(&raw mut (*pTabList).a as *mut SrcItem).offset(iSrc as isize))
                        .iCursor
                {
...
pub unsafe extern "C" fn sqlite3AuthCheck(
    ...
    unsafe {
        let mut db: *mut sqlite3 = (*pParse).db;
        let mut rc: ::core::ffi::c_int = 0;
        if (*db).xAuth.is_none() || (*db).init.busy as ::core::ffi::c_int != 0
...
#[no_mangle]
pub unsafe extern "C" fn sqlite3AuthContextPop(mut pContext: *mut AuthContext) {
    unsafe {
        if !(*pContext).pParse.is_null() {
            (*(*pContext).pParse).zAuthContext = (*pContext).zAuthContext;
            (*pContext).pParse = ::core::ptr::null_mut::<Parse>();
        }
    }
}
